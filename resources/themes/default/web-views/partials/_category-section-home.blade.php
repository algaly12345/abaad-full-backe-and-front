...
<style>
    .category-image {
        width: 50px !important; /* تغيير الحجم حسب الحاجة */
        height: 50px !important; /* تغيير الحجم حسب الحاجة */
        border-radius: 50% !important; /* لجعل الصورة دائرية */
        object-fit: cover !important; /* للتأكد من تغطية الصورة بالكامل داخل العنصر */
    }
</style>
                ...
                <div class="card __shadow h-100 max-md-shadow-0">
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            ...
                            <div>
                                <a class="text-capitalize view-all-text web-text-primary" href="{{ route('categories') }}">{{ translate('view_all') }}
                                ...
                                @foreach($categories as $key => $category)
                                    @if ($key < 10)
                                        <div class="text-center __m-5px __cate-item">
                                            <a href="javascript:void(0);" class="category-link" data-id="{{ $category['id'] }}">
                                                <div class="__img">
                                                    <img class="category-image" alt="{{ $category->name }}" src="{{ asset('storage/app/public/categories') }}/{{ $category['image'] }}">
                                                </div>
                        ...
                        <div id="estate-container" class="mt-4">
                            <!-- Fetched estate data will be displayed here -->
                        </div>
                    </div>
                </div>
            </div>
