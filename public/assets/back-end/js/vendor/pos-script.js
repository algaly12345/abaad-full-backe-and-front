...
$(".search-bar-input").on("keyup", function () {
    ...
    if (name.length > 0) {
        ...
        $.get({
            url: $("#route-user-products-search-business-chart").data("url"),
            dataType: "json",
            data: {
                name: name,
            },
...
$(".action-customer-change").on("change", function () {
    $.post({
        url: $("#route-user-pos-change-customer").data("url"),
        data: {
...
function viewAllHoldOrders(action = null) {
    ...
    $.post({
        url: $("#route-user-pos-view-hold-orders").data("url"),
        data: {
...
function renderQuickViewFunctionality() {
    $(".action-select-business-chart").on("click", function () {
        quickView($(this).data("id"));
    });
}

function renderQuickViewSearchFunctionality() {
    $(".action-select-search-business-chart").on("click", function () {
        quickView($(this).data("id"));
    });
}
...
function basicFunctionalityForCartSummary() {
    ...
    $(".action-clear-cart").on("click", () => {
        document.location.href = $("#route-user-pos-clear-cart-ids").data(
            "url"
        );
    });

    $(".action-new-order").on("click", () => {
        document.location.href = $("#route-user-pos-new-cart-id").data("url");
    });

    $(".action-cart-change").on("click", function () {
        let value = $(this).data("cart");
        let dynamicUrl = $("#route-user-pos-change-cart-editable").data(
            "url"
        );
    ...
    $(".action-empty-cart").on("click", function () {
        Swal.fire({
            ...
            if (result.value) {
                $.post(
                    $("#route-user-pos-empty-cart").data("url"),
                    {
                        _token: $('meta[name="_token"]').attr("content"),
                    },
                    function (data) {
...
$(".action-extra-discount").on("click", function (event) {
    ...
    if(discount.length === 0) {
        ...
        $.post({
            url: $("#route-user-pos-update-discount").data("url"),
            data: {
                discount: discount,
                type: type,
            },
...
$(".action-coupon-discount").on("click", function (event) {
    ...
    if(couponCode.length === 0) {
        ...
        $.post({
            url: $("#route-user-pos-coupon-discount").data("url"),
            data: {
...
function posUpdateQuantityFunctionality() {
    $(".action-pos-update-quantity").on("change", function (event) {
        let getKey = $(this).data("business-chart-key");
        let quantity = $(this).val();
        let variant = $(this).data("business-chart-variant");
...
function getPOSUpdateQuantity(key, qty, e, variant = null) {
    if (qty !== "") {
        $.post(
            $("#route-user-pos-update-quantity").data("url"),
            {
                _token: $('meta[name="_token"]').attr("content"),
                key: key,
                quantity: qty,
...
function updateQuantityResponseProcess(data) {
    if (data.productType === "physical" && data.qty < 0) {
        toastr.warning(
            $("#message-business-chart-quantity-is-not-enough").data("text"),
            {
                CloseButton: true,
                ProgressBar: true,
            }
    ...
    if (data.upQty === "zeroNegative") {
        toastr.warning(
            $("#message-business-chart-quantity-cannot-be-zero-in-cart").data("text"),
            {
                CloseButton: true,
                ProgressBar: true,
            }
    ...
    if (data.quantityUpdate == 1) {
        toastr.success($("#message-business-chart-quantity-updated").data("text"), {
            CloseButton: true,
            ProgressBar: true,
        });
    }
...
function renderViewHoldOrdersFunctionality() {
    $(".action-cancel-customer-order").on("click", function () {
        ...
        $.post({
            url: $("#route-user-pos-cancel-order").data("url"),
            data: {
...
function quickView(product_id) {
    $.ajax({
        url: $("#route-user-pos-quick-view").data("url"),
        type: "GET",
        data: {
...
function getVariantPrice(type = null) {
    if (
        ...
        $.ajax({
            type: "POST",
            url:
                $("#route-user-pos-get-variant-price").data("url") +
                (type ? "?type=" + type : ""),
...
function addToCart(form_id = "add-to-cart-form") {
    if (checkAddToCartValidity()) {
        ...
        $.post({
            url: $("#route-user-pos-add-to-cart").data("url"),
            ...
            success: function (data) {
                if (data.data == 1) {
                    ...
                    Swal.fire({
                        icon: "error",
                        title: $("#message-cart-word").data("text"),
                        text: $("#message-sorry-business-chart-is-out-of-stock").data(
                            "text"
                        ),
                    });
                    return false;
...
function removeFromCart() {
    $(".remove-from-cart").on("click", function () {
        ...
        $.post(
            $("#route-user-pos-remove-cart").data("url"),
            {
                _token: $('meta[name="_token"]').attr("content"),
                id: id,
                variant: variant,
...
function productStockMessage(type,){
    $('.business-chart-stock-message').empty().html($('#get-business-chart-stock-message').data(type))
...
function setProductData(parentClass,price,tax,discount){
    ...
    $('.'+parentClass+' '+'.set-business-chart-tax').html(tax);
