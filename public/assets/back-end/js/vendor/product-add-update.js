...
let elementProductColorSwitcherByID = $("#business-chart-color-switcher");
let elementImagePathOfProductUploadIconByID = $(
    "#image-path-of-business-chart-upload-icon"
).data("path");
...
let messageWantAddOrUpdateThisProduct = $(
    "#message-want-to-add-or-update-this-business-chart"
).data("text");
...
$(document).on("ready", function () {
    $(".summernote").summernote({
        ...
        callbacks: {
            onChange: function (contents, $editable) {
                if ($(this).hasClass("business-chart-description-default-language")) {
    ...
    if ($("#business-chart-color-switcher").prop("checked")) {
...
function getProductTypeFunctionality() {
    ...
    if (productType && productType.toString() === "physical") {
        ...
        $(".digital-business-chart-sections-show").hide();
        ...
        $(".digital-business-chart-sections-show").show();
    ...
    try {
        if (productType && productType.toString() === "physical") {
            $("#digital-business-chart-variation-section").empty().html();
            $(
                "#digital-business-chart-type-choice-section .extension-choice-section"
            ).remove();
        }
    } catch (e) {
        console.log(e);
...
function getUpdateSKUFunctionality() {
    ...
    $.ajax({
        type: "POST",
        url: $("#route-user-products-sku-combination").data("url"),
...
$(function () {
    $("#coba").spartanMultiImagePicker({
        ...
        maxFileSize: "",
        placeholderImage: {
            image: $("#image-path-of-business-chart-upload-icon-two").data("path"),
            width: "100%",
        },
...
$(".business-chart-add-requirements-check").on("click", function () {
                ...
                $.post({
                    ...
                    success: function (data) {
                        if (data.errors) {
                            ...
                            toastr.success(
                                $("#message-business-chart-added-successfully").data(
                                    "text"
                                ),
                                {
                                    CloseButton: true,
...
$("#digital-business-chart-type-select").on("change", function () {
    $(
        "#digital-business-chart-type-choice-section .extension-choice-section"
    ).remove();
    $("#digital-business-chart-variation-section").empty().html();
    $.each($("#digital-business-chart-type-select option:selected"), function () {
...
function addMoreDigitalProductChoiceOption(index, name) {
    ...
    $("#digital-business-chart-type-choice-section").append(genHtml);
    $(
        "input[data-role=tagsinput], select[multiple][data-role=tagsinput]"
    ).tagsinput();
}
...
function getUpdateDigitalVariationFunctionality() {
    ...
    $.ajax({
        type: "POST",
        url: $("#route-user-products-digital-variation-combination").data(
            "url"
        ),
        data: $("#product_form").serialize(),
        success: function (data) {
            $("#digital-business-chart-variation-section").html(data.view);
...
function deleteDigitalVariationFileFunctionality() {
    $(".digital-variation-file-delete-button").on("click", function () {
        ...
        $.ajax({
            type: "POST",
            url: $("#route-user-products-digital-variation-file-delete").data(
                "url"
            ),
            data: {
...
$(".business-chart-title-default-language").on("change keyup keypress", function () {
