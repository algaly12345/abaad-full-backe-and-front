    ...
    $.ajax({
        ...
        success: function (data) {
            switch (form.data("from")) {
                ...
                case "featured-business-chart-status":
                    toastr.success(
                        $("#get-featured-status-message").data("success")
                    );
                    break;
                case "business-chart-status-update":
                    if (data.success) {
                        toastr.success(updateText.data("text"));
                    } else {
                        toastr.error(updateText.data("error"));
...
$(".admin-business-chart-status-form").on("submit", function (event) {
    event.preventDefault();
    $.ajaxSetup({
        headers: {
            "X-CSRF-TOKEN": $('meta[name="_token"]').attr("content"),
...
$(".action-update-business-chart-quantity").on("click", function () {
    $.ajaxSetup({
        headers: {
            "X-CSRF-TOKEN": $('meta[name="_token"]').attr("content"),
        },
...
$(".reset-button").on("click", function () {
    $(".select-business-chart-button").text(
        $("#get-select-business-chart-text").data("text")
    );
});
...
$(".business-chart-gallery-filter").on("change", function () {
    let name = $(this).attr("name");
    let getData = $("#get-business-chart-gallery-route");
    let brandId = name == "brand_id" ? $(this).val() : getData.data("brand-id");
    let categoryId =
        name == "category_id" ? $(this).val() : getData.data("category-id");
    let vendorId =
        name == "vendor_id" ? $(this).val() : getData.data("user-id");
    let urlParams = [];
...
function productStockLimitStatus() {
    ...
    let getImage = $("#get-business-chart-stock-limit-image");
    let getTitle = $("#get-business-chart-stock-limit-title");
    let getMessage = $("#get-business-chart-stock-limit-message");
    let productStockView = $("#get-business-chart-stock-view");
    $.get({
        url: productStockAction,
        dataType: "json",
        success: function (response) {
            if (response.status === "one_product") {
                $(".business-chart-limited-stock-alert .image")
                    .attr("src", "")
                    .attr("src", response.thumbnail);
                $(".business-chart-limited-stock-alert .image").attr("width", 50);
                $(".business-chart-limited-stock-alert .title")
                    .empty()
                    .html(response.product.name);
                $(".business-chart-limited-stock-alert .message")
                    .empty()
                    .html(getMessage.data("message-for-one-business-chart"));
                $(".business-chart-limited-stock-alert .business-chart-list")
                    .attr("href", "")
                ...
                $(".business-chart-limited-stock-alert .message")
                    ...
                    .html(
                        response.product_count <= 100
                            ...
                            "+ " +
                            getMessage.data(
                                "message-for-three-plus-business-chart"
                            )
            ...
            if (response.product_count > 0) {
                $(".business-chart-limited-stock-alert").addClass("active");
            }
        },
    });
}
$(".business-chart-stock-limit-close").on("click", function () {
    $(".business-chart-limited-stock-alert").removeClass("active");
});
$(document).ready(function () {
    if (
        ...
        $(".business-chart-limited-stock-alert").hide();
    } else {
...
$(document).on("click", ".business-chart-stock-alert-hide", function () {
    document.cookie =
...
$(document).on("click", ".business-chart-stock-limit-close", function () {
    document.cookie =
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (
                ...
                $(".business-chart-restock-stock-alert").hide();
            } else {
                if (
...
$(document).on("click", ".business-chart-restock-request-alert-hide", function () {
    document.cookie = "6valley_restock_request_status=accepted; path=/";
    $(".business-chart-restock-stock-alert").hide();
});

function productRestockStockLimitStatus(response) {
    let mainElement = $(".business-chart-restock-stock-alert");
    ...
    mainElement.find(".business-chart-link").attr("data-link", response?.route);
    mainElement.addClass("active");
}

$(".business-chart-restock-stock-close").on("click", function () {
    document.cookie = "6valley_restock_request_status=accepted; path=/";
    $(".business-chart-restock-stock-alert").hide();
});
...
$(".clearance-business-chart-add-submit").on("click", function () {
    let form = $('.clearance-add-business-chart')[0];
    let formData = new FormData(form);
    ajaxSetupToken();
    $.post({
        url: $('.clearance-add-business-chart').attr("action"),
        data: formData,
        contentType: false,
        processData: false,
        beforeSend: function () {
        ...
        success: function (response) {
            if(response.status) {
                $('#business-chart-add-modal').modal('hide');
                toastr.success(response.message)
                console.log(response.redirect_url)
                window.location.href = response?.redirect_url;
            } else {
                $('#business-chart-add-modal').modal('show');
                toastr.error(response.message)
            }
        },
        complete: function () {
...
$('.discount-edit-btn').on('click', function () {
    var productId = $(this).data('business-chart-id');
    var id = $(this).data('id');
    var discountAmount = $(this).data('discount-amount');
    var imageUrl = $(this).data('image');
    var productName = $(this).data('business-chart-name');
