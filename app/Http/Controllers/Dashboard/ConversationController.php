...
class ConversationController extends Controller
{
    ...
    {
        $conversations = Conversation::with(['sender', 'receiver', 'last_message'])->WhereUserType('vendor');
        if($request->query('key')) {
            $key = explode(' ', $request->get('key'));
            $conversations = $conversations->where(function($qu)use($key){
                $qu->whereHas('sender',function($query)use($key){
                    foreach ($key as $value) {
                        $query->where('name', 'like', "%{$value}%")->orWhere('name', 'like', "%{$value}%")->orWhere('phone', 'like', "%{$value}%");
                    }
                })
                    ->orWhereHas('receiver',function($query1)use($key){
                        foreach ($key as $value) {
                            $query1->where('name', 'like', "%{$value}%")->orWhere('name', 'like', "%{$value}%")->orWhere('phone', 'like', "%{$value}%");
                        }
                    });
            });
        }
        ...
        if ($request->ajax()) {
            $view = view('dashboard.messages.data',compact('conversations'))->render();
    ...
    public function view($conversation_id,$user_id)
    {
        ...
        $receiver = Agent::find($user_id);
        // $user = User::find($receiver->user_id);
        $user = $receiver;
        return response()->json([
            'view' => view('dashboard.messages.partials._conversations', compact('convs', 'user', 'receiver'))->render()
        ]);
    }
