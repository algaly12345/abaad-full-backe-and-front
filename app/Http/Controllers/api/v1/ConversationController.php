...
class ConversationController extends Controller
{
    public function messages_store(Request $request)
    {
        ...
        if($request->conversation_id){
            ...
            if($request->receiver_type == 'admin'){
                ...
                $vendor = User::find($request->receiver_id);
                if(!$receiver){
                    $receiver = new UserInfo();
                    $receiver->user_id = $vendor->id;
                ...
                $receiver = UserInfo::where('user_id',$request->receiver_id)->first();
                $delivery_man = User::find($request->receiver_id);
        ...
        if(!$conversation){
            ...
            $conversation->estate_id = $request->estate_id;
            $conversation->save();
        ...
        if($conv->sender_type == 'vendor' && $conversation->sender){
            $vd = User::find($conv->sender->vendor_id);
            ...
            $vd = User::find($conv->receiver->vendor_id);
            // $order = Order::where('user_id',$request?->user()?->id)->count();
        }else if($conv->sender_type == 'delivery_man' && $conversation->sender){
            $user2 = User::find($conv->sender->deliveryman_id);
            ...
            $user2 = User::find($conv->receiver->deliveryman_id);
            // $order = Order::where('user_id',$request?->user()?->id)->where('delivery_man_id', $user2->id)->whereIn('order_status', ['pending','accepted','confirmed','processing','handover','picked_up'])->count();
        }
        else{
            $order=1;
    ...
    public function chat_image(Request $request)
    {
        ...
        $url = Storage::disk('public')->url('conversation/' . $image_name);
    ...
    public function conversations(Request $request)
    {
        ...
        $conversations = Conversation::with('sender','receiver','last_message','estate')
    ...
    public function messages(Request $request)
    {
        ...
        if($request->conversation_id){
            ...
            $vendor = UserInfo::where('user_id', $request->vendor_id)->first();

            if(!$vendor){
                $vd = User::find($request->vendor_id);
                $vendor = new UserInfo();
                $vendor->user_id = $vd->id;
                $vendor->name = $vd?->restaurants[0]?->name;
                $vendor->l_name = '';
            ...
            $dm = UserInfo::where('user_id', $request->delivery_man_id)->first();
            if(!$dm){
                $user2 = User::find($request->delivery_man_id);
                $dm = new UserInfo();
        ...
        if(isset($conversation)){
            if($conversation->sender_type == 'vendor' && $conversation->sender){
                $vd = User::find($conversation->sender->vendor_id);
                ...
                $vd = User::find($conversation->receiver->vendor_id);
                // $order = Order::where('user_id',$user->user_id)->where('restaurant_id', $vd?->restaurants[0]?->id)->whereIn('order_status', ['pending','accepted','confirmed','processing','handover','picked_up'])->count();
            }else if($conversation->sender_type == 'delivery_man' && $conversation->sender){
                $user2 = User::find($conversation->sender->deliveryman_id);
                ...
                $user2 = User::find($conversation->receiver->deliveryman_id);
                // $order = Order::where('user_id',$user->user_id)->where('delivery_man_id', $user2->id)->whereIn('order_status', ['pending','accepted','confirmed','processing','handover','picked_up'])->count();
            }
            else{
                $order=1;
    ...
    public function dm_messages_store(Request $request)
    {
        ...
        $offset = $request['offset']??1;
        $fcm_token_web = null;
        $dm = User::where(['auth_token' => $request['token']])->first();
    ...
    public function dm_conversations(Request $request)
    {
        ...
        $delivery_man = User::where(['auth_token' => $request['token']])->first();
